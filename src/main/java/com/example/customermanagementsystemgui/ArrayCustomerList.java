...
public class ArrayCustomerList {
    ...
    public boolean delete(String customer_name) {
        ...
        for(int i = 0; i <= top; i++)
            if(customers_array[i].getName().equals(customer_name)) {
                for(int j = i; j < top; j++)
    ...
    public void selection_sort_for_id() {
        for(int pass = 1; pass <= top; pass++) {
            ...
            while(index >= 0 && customers_array[index].getContract_id() < customers_array[pass].getContract_id()) {
    ...
    public void merge_sort_for_id(int i, int i1) {
    }
